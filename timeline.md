...
| 21 Aug | Fri | P1 S3.3 · Rebuild `index.css`, install Prettier                               | skipped           |     | No session — S3.7–3.8 rolled to Sat                                                                                       |
| 22 Aug | Sat | P1 S3.4–3.8 · `router.tsx`, `AppLayout`, `RouterProvider`, `useParams` guards | **S3.7–3.8 done** |     | useParams guards on ProjectPage + TaskPage, `*` catch-all, AppLayout polish                                               |

### Week 2 · 24–29 Aug — Phase 1: projects list & detail

| Date   | Day | Planned step                                      | Actual step          | Hrs | Note                                                                                       |
| ------ | --- | ------------------------------------------------- | -------------------- | --- | ------------------------------------------------------------------------------------------ |
| 24 Aug | Mon | P1 S4.1 · `Card` component                        | **S4.1 done**        |     | Card + Card.css + barrel, `cn` util, Button refactored to use `cn`, Prettier + format pass |
| 25 Aug | Tue | P1 S4.2 · ProjectsPage list (`map`, keys, `Link`) | **S4.2 in progress** |     | ProjectsPage heading shell, EmptyState stub started, CSS design tokens in Button           |
| 26 Aug | Wed | P1 S4.3 · `EmptyState` component                  | **S4.3 done**        |     | EmptyState (title, message, action slot), CSS + barrel, wired on ProjectsPage               |
| 27 Aug | Thu | P1 S4.4 · ProjectPage lookup by id + not-found    | skipped              |     | No session — S4.4 rolled to Sat                                                            |
| 28 Aug | Fri | P1 S5.1 · `Badge` component (status + priority)   | **revision**         |     | cn.ts + ClassValue types deep-dive, project-learnings updates, timeline sync; S5.1 pending |
...
| 25 Dec | Fri | P7.5 · Reducer test _(holiday — skip if needed)_          |             |     |      |
...
| 1 Jan  | Fri | P7.12 · Friendly 401 / 429 UI _(holiday — skip if needed)_ |             |     |      |
